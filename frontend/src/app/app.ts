...
export class App extends BaseComponent implements OnInit {
  ...
  override ngOnInit() {
    ...
    this.authService.licensed.subscribe((license: string) => {
      console.log("App 148: License state changed to: ", license);
      if (license !== 'ACTIVE') {
        console.log("G not licensed")
        this.isLicensed = false;
        return;
      }
      console.log("G Licensed...🥰")
  ...
  configurePanelMenu() {
    console.log("App 160: Loading menus...");
    if (!this.isLicensed) {
      console.log("Please go make a sub..");
